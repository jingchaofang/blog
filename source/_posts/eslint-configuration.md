...
ESLint默认使用Espree作为其解析器，你可以在配置文件中指定一个不同的解析器，只要该解析器符合下列要求：
...
- `browser` - browser global variables.浏览器环境中的全局变量。
- node - Node.js global variables and Node.js scoping.Node.js 全局变量和 Node.js 作用域。
- commonjs - CommonJS global variables and CommonJS scoping (use this for browser-only code that uses Browserify/WebPack).CommonJS 全局变量和 CommonJS 作用域 (一般用于 Browserify/WebPack 打包的只在浏览器中运行的代码)。
- shared-node-browser - Globals common to both Node.js and Browser.Node 和 Browser 通用全局变量。
- es6 - enable all ECMAScript 6 features except for modules (this automatically sets the ecmaVersion parser option to 6).启用除了 modules 以外的所有 ECMAScript 6 特性（该选项会自动设置 ecmaVersion 解析器选项为 6）。
- worker - web workers global variables.Web Workers 全局变量。
- amd - defines require() and define() as global variables as per the amd spec.将 require() 和 define() 定义为像 amd 一样的全局变量。
- mocha - adds all of the Mocha testing global variables.添加所有的 Mocha 测试全局变量。
- jasmine - adds all of the Jasmine testing global variables for version 1.3 and 2.0.添加所有的 Jasmine 版本 1.3 和 2.0 的测试全局变量。
- jest - Jest global variables.Jest 全局变量。
- phantomjs - PhantomJS global variables.PhantomJS 全局变量。
- protractor - Protractor global variables.Protractor 全局变量。
- qunit - QUnit global variables.QUnit 全局变量。
- jquery - jQuery global variables. jQuery 全局变量。
- prototypejs - Prototype.js global variables.Prototype.js 全局变量。
- shelljs - ShellJS global variables.ShellJS 全局变量。
- meteor - Meteor global variables.Meteor 全局变量。
- mongo - MongoDB global variables.MongoDB 全局变量。
- applescript - AppleScript global variables. AppleScript 全局变量。
- nashorn - Java 8 Nashorn global variables.Java 8 Nashorn 全局变量
- serviceworker - Service Worker global variables.Service Worker 全局变量。
- atomtest - Atom test helper globals.Atom 测试全局变量。
- embertest - Ember test helper globals.Ember 测试全局变量。
- webextensions - WebExtensions globals. WebExtensions 全局变量。
- greasemonkey - GreaseMonkey globals. GreaseMonkey 全局变量。
...
可以在源文件里、在配置文件中或使用命令行的 `--env` 选项来指定环境。
...
This example is the same as the last example, only it uses the numeric codes instead of the string values. The [eqeqeq](https://eslint.org/docs/rules/eqeqeq) rule is off and the [curly](https://eslint.org/docs/rules/curly) rule is set to be an error.
...
To disable all rules on a specific line, use a line comment in one of the 
following formats:
...
1. Inline configuration 行内配置
    ...
    3. `/*eslint*/`
    4. `/*eslint-env*/`
2. Command line options: 命令行选项：
    1. `--global`
    2. `--rule`
    3. `--env`
    4. `-c`, `--config`
3. Project-level configuration: 项目级配置：
    1. `.eslintrc.*` or `package.json` file in same directory as linted file。与要检测的文件在同一目录下的 `.eslintrc.*` 或 `package.json` 文件
    2. Continue searching for `.eslintrc` and `package.json` files in ancestor directories (parent has highest precedence, then grandparent, etc.), up to and including the root directory or until a config with `"root": true` is found.继续在父级目录寻找 `.eslintrc` 或 `package.json`文件，直到根目录（包括根目录）或直到发现一个有`"root": true`的配置。
    3. In the absence of any configuration from (1) thru (3), fall back to a personal default configuration in `~/.eslintrc`.如果不是（1）到（3）中的任何一种情况，退回到 `~/.eslintrc` 中自定义的默认配置
...
A [sharable configuration](https://cn.eslint.org/docs/developer-guide/shareable-configs) is an npm package that exports a configuration object. Make sure the package has been installed to a directory where ESLint can require it.

可共享的配置是一个 npm 包，它输出一个配置对象。要确保这个包安装在 ESLint 能请求到的目录下。
...
A [plugin](https://cn.eslint.org/docs/developer-guide/working-with-plugins) is an npm package that usually exports rules. Some plugins also export one or more named [configurations](https://cn.eslint.org/docs/developer-guide/working-with-plugins#configs-in-plugins). Make sure the package has been installed to a directory where ESLint can require it.

插件 是一个 npm 包，通常输出规则。一些插件也可以输出一个或多个命名的配置。要确保这个包安装在 ESLint 能请求到的目录下。
...
- Lines beginning with `#` are treated as comments and do not affect ignore patterns.以 `#` 开头的行被当作注释，不影响忽略模式。
- Paths are relative to `.eslintignore` location or the current working directory. This also influences paths passed via `--ignore-pattern`.路径是相对于 `.eslintignore` 的位置或当前工作目录。这也会影响通过 `--ignore-pattern`传递的路径。
- Ignore patterns behave according to the `.gitignore` specification.忽略模式同 .gitignore 规范
- Lines preceded by `!` are negated patterns that re-include a pattern that was ignored by an earlier pattern.以 `!`开头的行是否定模式，它将会重新包含一个之前被忽略的模式。
